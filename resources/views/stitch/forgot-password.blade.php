...
@section('title', 'Forgot Password - Kind')
    ...
    <div class="glass-card rounded-[24px] border border-white/50 p-container-padding md:p-stack-lg flex flex-col gap-stack-md relative overflow-hidden">
        ...
        <div class="flex flex-col items-center text-center gap-base">
            <div class="w-14 h-14 rounded-xl bg-gradient-to-br from-primary/10 to-primary/5 flex items-center justify-center mb-2 shadow-sm border border-white/60">
                <span class="material-symbols-outlined text-primary text-[32px] drop-shadow-sm">lock_reset</span>
            </div>
            <h1 class="font-headline-lg text-headline-lg text-on-surface">Reset Password</h1>
            <p class="font-body-sm text-body-sm text-on-surface-variant max-w-[280px]">Enter your email to receive a password reset link.</p>
        </div>
        <!-- Form -->
        <form method="POST" action="{{ route('password.email') }}" class="flex flex-col gap-stack-sm w-full mt-2">
            @csrf
            @if (session('status'))
